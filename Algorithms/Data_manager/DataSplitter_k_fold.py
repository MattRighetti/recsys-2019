...
class DataSplitter_k_fold(DataSplitter):
    ...
    def __init__(self, dataReader_object, n_folds=5, forbid_new_split=False, force_new_split=False):
        ...
        super(DataSplitter_k_fold, self).__init__(dataReader_object, forbid_new_split=forbid_new_split,
    ...
    def get_statistics_URM(self):
        ...
        for fold_index in range(self.n_folds):
            ...
            print(
                "\t Statistics for fold {}: n_interactions {} ( {:.2f}%), n_items {} ( {:.2f}%), density: {:.2E}".format(
                    fold_index,
    ...
    def get_holdout_split(self):
        ...
        assert self.n_folds >= 3, "DataSplitter: To get a holdout split URM_train, URM_validation, URM_test, the splitter must have at least 3 folds, currently it has {}".format(
            self.n_folds)
...
class DataSplitter_ColdItems_k_fold(DataSplitter_k_fold):
    ...
    def _select_balanced_split(self, URM):
        ...
        print("DataSplitter_ColdItems_k_fold: Smallest fold size over max fold size: {:.0f} %".format(
            best_balanced_split_ratio * 100))
