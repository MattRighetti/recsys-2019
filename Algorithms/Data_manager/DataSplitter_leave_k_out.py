...
class DataSplitter_leave_k_out(DataSplitter):
    ...
    def __init__(self, dataReader_object, k_out_value=1, forbid_new_split=False, force_new_split=False,
        ...
        super(DataSplitter_leave_k_out, self).__init__(dataReader_object, forbid_new_split=forbid_new_split,
    ...
    def _load_from_DataReader_ICM_and_mappers(self):
        ...
        for ICM_name in self.SPLIT_ICM_DICT.keys():
            self.SPLIT_ICM_MAPPER_DICT[
                ICM_name] = self.get_dataReader_object().get_ICM_feature_to_index_mapper_from_name(ICM_name)
