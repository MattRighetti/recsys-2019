...
def assert_URM_ICM_mapper_consistency(URM_DICT, GLOBAL_MAPPER_DICT, ICM_DICT, ICM_MAPPER_DICT, DATA_SPLITTER_NAME):
    ...
    assert len(set(user_original_ID_to_index.values())) == len(
        user_original_ID_to_index), "user it-to-index mapper values do not have a 1-to-1 correspondance with the key"
    assert len(set(item_original_ID_to_index.values())) == len(
        item_original_ID_to_index), "item it-to-index mapper values do not have a 1-to-1 correspondance with the key"

    assert n_users == len(
        user_original_ID_to_index), print_preamble + "user it-to-index mapper contains a number of keys different then the number of users"
    assert n_items_URM == len(
        item_original_ID_to_index), print_preamble + "item it-to-index mapper contains a number of keys different then the number of items"

    assert n_users >= max(
        user_original_ID_to_index.values()), print_preamble + "user it-to-index mapper contains indices greater than number of users"
    assert n_items_URM >= max(
        item_original_ID_to_index.values()), print_preamble + "item it-to-index mapper contains indices greater than number of item"
    ...
    assert np.isin(nonzero_items, np.array(list(
        item_original_ID_to_index.values()))).all(), print_preamble + "there exist items with interactions that do not have a mapper entry"

    URM_all = sps.csr_matrix(URM_all)
    nonzero_users_mask = np.ediff1d(URM_all.indptr) > 0
    nonzero_users = np.arange(0, n_users, dtype=np.int)[nonzero_users_mask]
    assert np.isin(nonzero_users, np.array(list(
        user_original_ID_to_index.values()))).all(), print_preamble + "there exist users with interactions that do not have a mapper entry"

    if ICM_MAPPER_DICT is not None:

        assert len(ICM_DICT) == len(
            ICM_MAPPER_DICT), print_preamble + "The available ICM and the available ICM mappers do not have the same length. ICMs are {}, mappers are {}".format(
            len(ICM_DICT), len(ICM_MAPPER_DICT))

        assert all(ICM_name in ICM_MAPPER_DICT for ICM_name in
                   ICM_DICT.keys()), print_preamble + "Not all ICM sparse matrix have a corresponding ICM mapper"
        assert all(ICM_name in ICM_DICT for ICM_name in
                   ICM_MAPPER_DICT.keys()), print_preamble + "Not all ICM mappers have a corresponding ICM sparse matrix"
        ...
        for ICM_name, ICM_object in ICM_DICT.items():
            ...
            assert n_items_ICM == n_items_URM, print_preamble + "Number of items in ICM {} is {} while in URM is {}".format(
                ICM_name, n_items_ICM, n_items_URM)
            ...
            assert n_features >= len(
                feature_original_id_to_index), print_preamble + "feature id-to-index mapper contains more keys than features in ICM {}".format(
                ICM_name)
            assert n_features >= max(
                feature_original_id_to_index.values()), print_preamble + "feature id-to-index mapper contains indices greater than number of features in ICM {}".format(
                ICM_name)
            ...
            nonzero_items = np.arange(0, n_items_URM, dtype=np.int)[nonzero_items_mask]
            assert np.isin(nonzero_items, np.array(list(
                item_original_ID_to_index.values()))).all(), print_preamble + "there exist items with features that do not have a mapper entry in ICM {}".format(
                ICM_name)
            ...
            assert np.isin(nonzero_features, np.array(list(
                feature_original_id_to_index.values()))).all(), print_preamble + "there exist users with interactions that do not have a mapper entry in ICM {}".format(
                ICM_name)
