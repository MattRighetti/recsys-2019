...
from Algorithms.Data_manager.data_consistency_check import assert_URM_ICM_mapper_consistency
...
class DataReader(object):
    ...
    def __init__(self, reload_from_original_data=False, ICM_to_load_list=None):
        ...
        if ICM_to_load_list is None:
            ...
            assert all([ICM_to_load in self.AVAILABLE_ICM for ICM_to_load in ICM_to_load_list]), \
                "{}: ICM_to_load_list contains ICM names which are not available for the current DataReader".format(
    ...
    def _assert_is_initialized(self):
        assert self._LOADED_URM_DICT is not None, "DataReader {}: Unable to load data split. The split has not been generated yet, call the load_data function to do so.".format(
    ...
    def _load_from_original_file(self):
        ...
        raise NotImplementedError(
            "{}: _load_from_original_file was not implemented for the required dataset. Impossible to load the data".format(
                self._DATA_READER_NAME))
    ...
    def _get_dataset_name_root(self):
        ...
        """
        raise NotImplementedError(
            "{}:_get_dataset_name_root was not implemented for the required dataset. Impossible to load the data".format(
                self._DATA_READER_NAME))
