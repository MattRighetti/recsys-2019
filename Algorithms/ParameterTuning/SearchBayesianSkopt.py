...
from Algorithms.ParameterTuning.SearchAbstractClass import SearchAbstractClass, writeLog, get_result_string_evaluate_on_validation
...
class SearchBayesianSkopt(SearchAbstractClass):
    ...
    def search(self, recommender_input_args,
        ...
        assert save_model in ["no", "all",
                              "best"], "{}: parameter save_model must be in '['no', 'all', 'best']', provided was '{}'.".format(
        ...
        writeLog("{}: Search complete. Best config is {}: {}\n".format(self.ALGORITHM_NAME, self.best_solution_counter,
    ...
    def _evaluate_on_test_with_data_last(self):
        ...
        print("{}: Evaluation with constructor data for final test. Using best config:".format(self.ALGORITHM_NAME),
              self.best_solution_parameters)
        ...
        result_dict_test, result_string, evaluation_test_time = self._evaluate_on_test(recommender_instance,
                                                                                       print_log=False)
    ...
    def _objective_function(self, current_fit_parameters_dict):
        ...
        try:
            ...
            if self.save_metadata:
                ...
                self.metadata_dict["time_on_train_avg"] = self.metadata_dict["time_on_train_total"] / (
                            self.model_counter + 1)
                self.metadata_dict["time_on_validation_avg"] = self.metadata_dict["time_on_validation_total"] / (
                            self.model_counter + 1)
            ...
            if self.save_model == "all":
                ...
                recommender_instance.saveModel(self.output_folder_path,
                                               file_name=self.output_file_name_root + "_model_{}".format(
                                                   self.model_counter))
            ...
            if self.best_solution_val == None or self.best_solution_val < result_dict[self.metric_to_optimize]:
                ...
                if self.evaluator_test is not None:
                    result_dict_test, _, evaluation_test_time = self._evaluate_on_test(recommender_instance,
                                                                                       print_log=True)
                    ...
                    if self.save_metadata:
                        ...
                        self.metadata_dict["time_on_test_avg"] = self.metadata_dict[
