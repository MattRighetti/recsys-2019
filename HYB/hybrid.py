...
class HybridRecommender(object):
    ...
    def recommend(self, user_id, at=10, exclude_seen=False):
        self.userBCF_scores = self.userBCF.recommend(user_id, exclude_seen=exclude_seen)
        self.itemBCF_scores = self.itemBCF.recommend(user_id, exclude_seen=exclude_seen)

        scores = (self.userBCF_scores * self.userBCF_w) + (self.itemBCF_scores * self.userBCF_w)
